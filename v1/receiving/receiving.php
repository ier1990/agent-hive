...
ini_set('error_log','/web/private/logs/receiving-error.log');
...
try {
    ...
    if ($method === 'GET') {
        ...
        // Build a “data” array so we can reuse getDatabasePath()
        $dataForDb = [];
        if ($dbParam !== null)      $dataForDb['db'] = $dbParam;
        if (isset($_GET['service'])) $dataForDb['service'] = $_GET['service'];
...
try {
  ...
  if (!is_array($data)) {
    http_response_code(400);
    echo json_encode(['error' => 'invalid_json', 'req_id' => $reqId], JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
    exit;
  }
...
function dbg_log(array $hdr, $raw, $ok=true, $note='') {
  ...
  @file_put_contents('/web/private/logs/receiving-debug.log', $line . "\n", FILE_APPEND);
}
