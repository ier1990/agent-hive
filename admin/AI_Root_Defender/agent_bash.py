...
def run_shell() -> int:
    ...
    # Initialize notes handler and memory loader
    ...
    # Initialize AI provider (optional — shell works without it)
    # Support both new providers-list format and legacy name/slug format.
    slug, entry_cfg, provider_label, active_idx, providers_list = get_active_provider_entry(settings)
    # Override with CLI --provider arg if set.
    _cli_provider = os.environ.get("ROOT_GUARDIAN_PROVIDER", "").strip()
    if _cli_provider:
        try:
            active_idx, entry_cfg = resolve_provider_by_ref(providers_list, _cli_provider)
            slug = str(entry_cfg.get("name", slug))
            provider_label = str(entry_cfg.get("label", slug))
        except ValueError as exc:
            print(f"warning: --provider {_cli_provider!r}: {exc}")
    provider_name = slug
    ...
    try:
        ...
        print(f"warning: provider '{provider_name}' not available — {exc}")
        print("         Natural language input disabled. Slash commands still work.")
    except Exception as exc:
        print(f"warning: provider init failed ({exc}). Natural language disabled.")
    ...
    _setup_readline(logs_dir / "guardian_shell_history.log")
            ...
            if treat_as_command and line.startswith("/contract "):
                ...
                if is_final:
                    if turn_count >= max_turns:
                        print("session reached max turns")
                        logger.end_session("Session ended at max turns.")
                        return 0
...
def main() -> int:
    ...
    parser.add_argument("--non-interactive", action="store_true", help="Reserved for future batch mode")
    ...
    if args.non_interactive:
        print("non-interactive mode is not implemented yet")
        return 1
