...
# example usage:
#   ./sysinfo.sh https://localhost/v1/inbox/


# Argument: API_URL (required)
URL="${1:-}"
if [[ -z "$URL" ]]; then
...
# Argument: IS_PUBLIC (optional, default: false)
# Indicates if server is public-facing (true) or behind firewall (false)
IS_PUBLIC="${2:-false}"


echo -e "Collecting system info and sending to $URL...\n"

# Ensure jq is installed
if ! command -v jq >/dev/null 2>&1; then
  echo "ERROR: jq not found. Please install jq manually."
  echo "For example: apt-get install jq   # Debian/Ubuntu" >&2
  exit 1
fi
