...
$chat_json = '/web/private/codewalker.json';
$db_chat = '/web/private/db/chat.db';

// Simple API Chat Console for admin use
...
$defaultChatUrl = rtrim($origin, '/') . '/v1/chat/';
$defaultChatAsyncUrl = rtrim($origin, '/') . '/v1/chat/?async=1';

$defaultUrl = isset($_GET['url']) ? (string)$_GET['url'] : $defaultChatUrl;
...
<head>
	...
	<title>API AI Chat Tester</title>
	<style>
		...
		.btn { padding: 8px 12px; border: 1px solid #444; background: #111; color: #fff; border-radius: 6px; cursor: pointer; text-decoration: none; display: inline-block; }
...
<body>
	<h1>API AI Chat Tester</h1>
	<div class="note">
		Use this tool to test the chat endpoint. Default routes to <code>/v1/chat/</code> (autoselector).
		<div style="margin-top:6px;">
			Quick: <a href="?url=<?php echo h($defaultChatUrl); ?>">Use sync</a>
			| <a href="?url=<?php echo h($defaultChatAsyncUrl); ?>">Use async</a>
		</div>
	</div>
	...
	<form method="post" class="card" autocomplete="off">
		...
		<div class="row">
			...
			<input type="text" id="url" name="url" value="<?php echo h($defaultUrl); ?>" placeholder="/v1/chat/">
		</div>
		<div class="grid">
			<div>
