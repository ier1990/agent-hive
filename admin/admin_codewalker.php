...
// Requirements: PHP 7.3+ with SQLite (pdo_sqlite)
...
if (!$DB_PATH || !is_string($DB_PATH)) {
    echo "Missing db_path in settings DB.";
    exit;
}

// Limit file writes to this root for safety
$WRITE_ROOT = $cfg['write_root'] ?? null;
if (!$WRITE_ROOT || !is_string($WRITE_ROOT)) {
    echo "Missing write_root in settings DB.";
    exit;
}
...
function cw_is_assoc_array($arr): bool
{
...
function cw_json_value_to_markdown($val, int $indent = 0): string
{
    $pad = str_repeat('  ', max(0, $indent));

    if (is_array($val)) {
        $out = '';
        if (cw_is_assoc_array($val)) {
            foreach ($val as $k => $v) {
                $k = (string)$k;
                if (is_array($v)) {
                    $out .= $pad . '- **' . $k . ':**\n' . cw_json_value_to_markdown($v, $indent + 1);
                } else {
                    $s = (string)$v;
                    $s = trim($s);
                    $out .= $pad . '- **' . $k . ':** ' . $s . "\n";
    ...
    $s = trim((string)$val);
    if ($s === '') return $pad . "(empty)\n";
    // If multiline, preserve in a code fence to keep formatting.
    if (strpos($s, "\n") !== false) {
        return $pad . "```\n" . $s . "\n```\n";
    }
    return $pad . $s . "\n";
...
function cw_summary_to_markdown(string $summary): string
{
    $raw = trim($summary);
    if ($raw === '') return '';
    ...
    // Special-case runner output: {"raw": "..."}
    if (isset($data['raw']) && count($data) === 1) {
        $only = (string)$data['raw'];
        return trim($only) !== '' ? (string)$only : $summary;
    }

    $md = "";
    if (cw_is_assoc_array($data)) {
        foreach ($data as $k => $v) {
            $k = (string)$k;
            $md .= '### ' . $k . "\n";
            $md .= cw_json_value_to_markdown($v, 0) . "\n";
        }
        return trim($md);
    }

    // Root JSON list
    return trim(cw_json_value_to_markdown($data, 0));
}
...
// Ensure core CodeWalker tables exist
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($op === 'run_on_file') {
        ...
        $action = strtolower((string)postp('action', ''));

        if ($path === '') {
            $flash = 'Missing path.';
        } elseif ($action !== 'summarize' && $action !== 'rewrite') {
            $flash = 'Invalid action.';
        } else {
            try {
                $res = cw_run_on_file($path, $action);
                $actionId = isset($res['action_id']) ? (int)$res['action_id'] : 0;
                $status = isset($res['status']) ? (string)$res['status'] : '';
                $err = isset($res['error']) ? (string)$res['error'] : '';

                if ($actionId > 0 && $status === 'ok') {
                    header('Location: ?view=action&id=' . $actionId);
                    exit;
                }

                $flash = 'Run failed: ' . h($err !== '' ? $err : $status);
            } catch (Throwable $e) {
                $flash = 'Run failed: ' . h($e->getMessage());
            }
        }
    }
    if ($op === 'apply_rewrite') {
...
echo '<script src="https://cdn.jsdelivr.net/npm/marked@9.1.6/marked.min.js"></script>';
echo '</head><body>';
...
elseif ($view === 'file') {
    ...
    if ($path==='') { echo '<div class="card">No file path provided.</div>'; }
        ...
        echo '<div id="run" style="margin-top:.75rem">';
        echo '<h4>Run</h4>';
        echo '<form method="post" style="display:flex;gap:.5rem;flex-wrap:wrap">';
        echo '<input type="hidden" name="csrf" value="'.h($csrf).'">';
        echo '<input type="hidden" name="op" value="run_on_file">';
